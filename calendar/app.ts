...
class MonthData extends Date {
  constructor(data: Data) {
    super(
      `${data.y.toString().padStart(4, "0")}-${data.m
        .toString()
        .padStart(2, "0")}-${data.d.toString().padStart(2, "0")}`
    );
  }
  ...
  readonly datesOfMonth = (() => {
    ...
    const temp = new Date(
      `${Math.floor(temp1 / 12)
        .toString()
        .padStart(4, "0")}-${((temp1 % 12) + 1).toString().padStart(2, "0")}-01`
    );
    temp.setDate(0);
    return temp.getDate();
  })();
...
class DateData extends MonthData {
  ...
  readonly shortNameOfMonth = new Intl.DateTimeFormat("en-US", {
    month: "short",
  }).format(this);
  readonly nameOfDay = new Intl.DateTimeFormat("en-US", {
    weekday: "long",
  }).format(this);
...
function initialize() {
  ...
  function setEventListener() {
    const HTML = {
      gotoPrevYear: document.querySelector<HTMLSpanElement>(
        "#goto-previous-year"
      ),
      gotoNextYear: document.querySelector<HTMLSpanElement>("#goto-next-year"),
      gotoPrevMonth: document.querySelector<HTMLSpanElement>(
        "#goto-previous-month"
      ),
      gotoNextMonth:
        document.querySelector<HTMLSpanElement>("#goto-next-month"),
    ...
    HTML.gotoAnywhen.addEventListener("input", () => {
      ...
      dataOfSelected.y = Math.max(
        Math.min(parseInt(temp.substring(0, 4)), 9999),
        1
      );
      dataOfSelected.m = Math.max(
        Math.min(parseInt(temp.substring(5, 7)), 12),
        1
      );
      setCalendar();
    });
    ...
    for (let i = 0; i < 6; i++) {
      for (let j in week) {
        const tableCell = document.querySelector<HTMLTableCellElement>(
          `#w${i + 1} .${week[j]}`
        );
        tableCell.addEventListener("click", () => {
          const data = document.querySelector<HTMLDivElement>(
            `#w${i + 1} .${week[j]}` + " div"
          ).innerHTML;
          if (!(data === "")) {
            erasePrevColor();
  ...
  function changeDataOfSelected(amount: number) {
    ...
    const changedDatesOfMonth = new MonthData({
      y: dataOfSelected.y,
      m: dataOfSelected.m,
      d: 1,
    }).datesOfMonth;
...
function erasePrevColor() {
  ...
  document.querySelector<HTMLTableCellElement>(
    `#w${selected.weekIdx} .${selected.shortNameOfDay}`
  ).style.backgroundColor = "#FFF";
  document.querySelector<HTMLTableCellElement>(
    `#weekdays .${selected.shortNameOfDay}`
  ).style.backgroundColor = "#FFF";
}

function setCalendar() {
  //Enabled if pages in the calendar are turned over
  ...
  handleDateSelected(
    ...
    selected.shortNameOfDay,
    dataOfSelected.d.toString()
  );
  ...
  function fillDateField(idxOf1st: number, date: number) {
    ...
    for (let i = 0; i < date; i++) {
      const css = `#w${Math.ceil((idxOf1st + i + 1) / 7)} .${
        week[(idxOf1st + i) % 7]
      }`;
      document.querySelector<HTMLDivElement>(css + " div").innerHTML = (
        i + 1
      ).toString();
    }
  }
...
function handleDateSelected(
  ...
  const selected = new DateData(dataOfSelected);
  const infoOfSelected = `${selected.nameOfDay} | ${
    selected.shortNameOfMonth
  }. ${selected.ordinalDate}, ${selected.getFullYear()}`;
  document.querySelector<HTMLSpanElement>("#info-selected").innerHTML =
    infoOfSelected;
}
