...
const boardElement = <HTMLCanvasElement>document.getElementById("board");
const ctx = <CanvasRenderingContext2D>boardElement.getContext("2d");
 ...
 * @param r Rate
 */
function show(w: number, h: number, x: number, y: number, r: number) {
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(r * x, 0);
...
function showEnd(w: number, h: number, x: number, y: number) {
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(x, 0);
    ctx.lineTo(x, y - m);
