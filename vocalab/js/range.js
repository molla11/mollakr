...
const ranges = isTrusted
  ? localRanges
  : Array.from({ length: wordMap.size }, () => false);
...
function showStatusToCheckbox() {
  ...
  const checkboxEls = document.querySelectorAll(
    `#range-checkbox-container input[type="checkbox"]`
  );
...
function setWordIndices() {
  ...
  console.log(
    `wordIndices has been refreshed. (${
      endTimeInWordIndices - startTimeInWordIndices
    }ms)`
  );
}
